    ...
    <div class="app-container">
      <vxe-form
        ...
        <vxe-form-item
          ...
          :item-render="{name: 'AInput'}"></vxe-form-item>
        <vxe-form-item
          field="role"
          title="AAutoComplete"
          span="8"
        ...
        <vxe-form-item
          ...
          align="center"
          collapse-node
          :item-render="{name: 'AButtons', children: [{ content: '查询', props: {type: 'primary', htmlType: 'submit'} }, { content: '重置', props: {htmlType: 'reset'} }]}"></vxe-form-item>
      </vxe-form>

      <vxe-toolbar export print custom size="small" ref="xToolbar">
        <template #buttons>
          <a-button size="small" @click="insertEvent">新增</a-button>
          <a-button size="small" @click="saveEvent">保存</a-button>
          <a-button size="small" @click="vaildEvent">校验</a-button>
          <a-dropdown :trigger="['click']" size="small" >
            <a-button size="small" >操作
              <a-icon type="down" />
            </a-button>
            <a-menu size="small" slot="overlay">
              <a-menu-item key="0">
      ...
      <div style="overflow: hidden;" :style="{ height: tableHeight}">
        <vxe-table
          ...
          <vxe-table-column
            ...
            title="AInput"
            min-width="140"
            :edit-render="{name: 'AInput'}"></vxe-table-column>
          <vxe-table-column
            field="role"
            title="AAutoComplete"
            width="160"
            :edit-render="{name: 'AAutoComplete', props: ACProps, events: {search: roleSearchEvent}}"></vxe-table-column>
          <vxe-table-column
            field="age"
            title="AInputNumber"
            width="160"
            :edit-render="{name: 'AInputNumber', props: {max: 35, min: 18}}"></vxe-table-column>
          <vxe-table-column
            field="sex"
            title="ASelect"
            width="140"
            :edit-render="{name: 'ASelect', options: sexList}"></vxe-table-column>
          <vxe-table-column
            field="region"
            title="ACascader"
            width="200"
            :edit-render="{name: 'ACascader', props: {options: regionList}}"></vxe-table-column>
          <vxe-table-column
            field="date1"
            title="ADatePicker"
            width="200"
            :edit-render="{name: 'ADatePicker', props: {type: 'date', format: 'YYYY/MM/DD'}}"></vxe-table-column>
          <vxe-table-column
            field="flag"
            title="ASwitch"
            width="100"
            :cell-render="{name: 'ASwitch'}"></vxe-table-column>
          <vxe-table-column
            field="rate"
            title="ARate"
            width="200"
            fixed="right"
            :cell-render="{name: 'ARate'}"></vxe-table-column>
        </vxe-table>
      </div>
      <a-pagination
        size="small"
...
import CreateUnit from '@/views/system/organization/modules/CreateUnit'
export default {
  data () {
    return {
      loading: false,
  ...
  mounted () {
    this.findList()
    this.tableHeight = (document.body.clientHeight - 64 - 96 - this.$refs.xToolbar.$vnode.elm.offsetHeight - this.$refs.xForm.$vnode.context.$vnode.elm.offsetHeight) + 'px'
  },
  methods: {
    findList () {
      this.loading = true
